...
export class Player extends Phaser.Physics.Arcade.Image {
  ...
  private _shiftDownAt = 0;
  private _shiftWasDown = false;
  private _reformTriggeredThisHold = false;
  ...
  isHoldingShift(): boolean {
    return this._shift.isDown && this._shiftDownAt > 0;
  }

  getShiftHoldMs(now: number): number {
    if (!this._shift.isDown || this._shiftDownAt === 0) return 0;
    return now - this._shiftDownAt;
  }
  ...
  update(): void {
    ...
    // Shift hold/release: tap = dash (on release), hold >= 220ms = reform (during hold)
    const shiftDown = this._shift.isDown;
    if (shiftDown && !this._shiftWasDown) {
      this._shiftDownAt = now;
      this._reformTriggeredThisHold = false;
    }
    ...
    // On release: tap → dash (only if reform wasn't triggered this hold)
    if (!shiftDown && this._shiftWasDown && this._shiftDownAt > 0) {
      this._shiftDownAt = 0;
      if (!this._reformTriggeredThisHold) {
